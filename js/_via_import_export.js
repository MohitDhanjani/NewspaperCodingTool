...
_via_import_export.prototype.export_to_nct_csv = function() {
  return new Promise( function(ok_callback, err_callback) {
    var csv = [];

    var csvHeader = [];

    csvHeader.push('NewsItemNo');
    ...
    for ( var aid in this.d.store.attribute ) {
      //attribute[aid] = this.d.store.attribute[aid].aname;
      csvHeader.push(this.d.store.attribute[aid].aname);
    }

    //csvHeader.push('RegionShape');
    csvHeader.push('RegionDetails');
    csvHeader.push('Filename');
    ...
    for ( var mid in this.d.store.metadata ) {
      ...
      // this.d.store.metadata[mid].av.forEach((element) => {
      //   line.push(_via_util_escape_quote_for_csv(element));
      // });
      //line.push( _via_util_obj2csv(this.d.store.metadata[mid].z) );
      line.push( _via_util_obj2csv(this.d.store.metadata[mid].xy) );
      //line.push( _via_util_obj2csv(this.d.store.metadata[mid].av) );
